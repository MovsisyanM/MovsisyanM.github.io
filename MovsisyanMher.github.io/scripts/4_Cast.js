...
const Cast = () => {
    ...
    M(canvas.width / 2, canvas.height / 3.5, canvas.width * (Details.size + (Device.phoneLayout ? 0.015 : 0)), rgba(Theme[1]), Details.Qx, -Details.Qy, Details.Qz, context).draw();
    Device.phoneLayout ? context.font = canvas.width / 14 + 5 * Math.sin(frameCount / 240) + "px Arial" : context.font = canvas.height / 20 + 5 * Math.sin(frameCount / 240) + "px Arial";
    context.fillStyle = rgba(Theme[2]); // Movsisyan's Gray
